...
def cross_section(matrix_ratecoef, delta_E, epsilon):
    ...
    a = factor * np.power(math.e,a) * matrix_ratecoef[5]*constants.electron_volt*10**24#barn and ev
    return np.sum(a)


a = np.linspace(0, 10* constants.electron_volt, 5000)
rate = np.zeros((5000,2))
for i in range(len(rate)):
    rate[i][1] = cross_section(matrix_ratecoef, 0.1 * constants.electron_volt, a[i])
    rate[i][0]=a[i]/constants.electron_volt


np.savetxt("arr_cross_strontum_5.csv", rate, delimiter=" ")