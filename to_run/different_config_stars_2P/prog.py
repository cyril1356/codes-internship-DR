...
def give_electron_config_stars(closed_config, transition, end_stars):
    ...
    for i in range(int(value_n_bound) + 1, end_stars + 1):
        list_bound.append(transition + " " + str(i) + "*1")
    return list_bound
...
list_confi = [5, 6, 7, 8, 9, 10, 11]

fac.Config("d", "5p %s*1" % (list_confi[compteur] + 1))

closed_config = "1s 2s 2p 3s 3p 4s 3d 4p"

final_config = give_electron_config_stars(closed_config, "5p", list_confi[compteur])
